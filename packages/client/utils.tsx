...
export const catmullRomToBezier = (
  ...
  for (let i = 0; i < points.length - 1; i++) {
    ...
    const b1x = p1.position.x + ((p2.position.x - p0.position.x) * tension) / 6;
    const b1y = p1.position.y + ((p2.position.y - p0.position.y) * tension) / 6;
    const b2x = p2.position.x - ((p3.position.x - p1.position.x) * tension) / 6;
    const b2y = p2.position.y - ((p3.position.y - p1.position.y) * tension) / 6;
